...
/*   Updated: 2025/03/03 15:56:40 by ineimatu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ray_trace(t_data *data, int x, double increment)
{
	...
	radians = data->ray->angle_start * (PI / 180.0);
	...
	if (data->ray->angle_start >= 270.0 && data->ray->angle_start <= 271.0)
	{
		data->ray->angle_start += 1.0;
		data->ray->angle_end += 1.0;
...
{ 
	...
	init_ray(data);
	printf("simonsimon\n");
	//ray->angle_start = adjust_angle(ray->angle_start);
	///printf("angle start %f\n, angle end %f\n,", data->ray->angle_start, data->ray->angle_end);
	...
	/*if (data->ray->angle_start >= 269.0 && data->ray->angle_start <= 271.0)
	{
		data->ray->angle_start += 1.0;
		data->ray->angle_end += 1.0;
		//printf("angle start %f\n, angle end %f\n,", data->ray->angle_start, data->ray->angle_end);
	...
	{
		//printf("Hola!\n");
		while (data->ray->angle_start >= 0.0000)
		{	
			ray_trace(data, x, increment);
			x++;
		}
		if (data->ray->angle_start <= 0.0000)
			data->ray->angle_start = data->ray->angle_start + 360.0;
